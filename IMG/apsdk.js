(function () {
    window.AD = {
        ...
        //adZoneId:null,
        adZoneIdKey: {},
        adzoneIdArr: ["1", "9", "10", "21", "22", "18", "19", "12", "13", "15", "16", "24", "25", "27", "28", "30", "31"],
        ...
        getADData: function (pl, adzoneId, info, callback) {
            ...
            var isFocus = false;
            for (var i = 0; i < AD.adzoneIdArr.length; i++) {
                if (AD.adzoneIdArr[i] == adzoneId) {
                    isFocus = true;
                }
            }
            var isText = false;
            ...
            $.getJSON(dataUrl + splitChar + 'pl=' + pl + '&adzones=' + adzoneId + '&vvid=' + uuid + '_' + mt + '&cuid=' + uuid + '&t=' + t + '&res=json&j=AD.resolveAD&callback=?');
         ...
         * type：1，2，3，4
         * 1 impression  广告曝光监测
         * 2 clickThrough  广告点击跳转
         * 3 clickTracking  广告点击检测
         * 4 trackingEvent  广告事件监测
         * @param im 1为主广告 0伴随广告
         */
        ...
        getAttachParamsUrl: function (url, uid, type, pl) {
            ...
            if (url.indexOf(AD.config.hostDomain) > -1 || url.indexOf("http://aes.acfun.cn") > -1) {
                ...
                var now = new Date().getTime(),
                    cuid = AD.config.triggerData.cuid;
                var tc = AD.config.triggerData.origintime + Math.ceil((now - AD.config.triggerData.clienttime) / 1000);
                var param = {
                    rt: type,
                    oid: adItem.oid,
                    ...
                    t: tc,
                    cuid: cuid,
                    data: [uid, pl || AD.config.triggerData.pl, AD.config.triggerData.vvid, adItem.adId, adItem.orderid, adItem.lc, adItem.ct, '1', now].join(",")
                };

                if (isNaN(param.t)) {
                    param.t = AD.config.triggerData.origintime || now;
                }
